...
1. Upload a new version of any [Entity](../../content/entities.md) they own (such as their profile).
...
### Constructing a Chain <a href="#constructing" id="constructing"></a>
...
| Field       | Value                                                                 |
| ----------- | --------------------------------------------------------------------- |
| `type`      | The name of a type ([see below](authchain.md#types)).                 |
| `payload`   | A type-dependent string.                                              |
| `signature` | The hex-encoded Ethereum signature of `payload`, beginning with `0x`. |

{% hint style="info" %}
Since the most common serialization of an authentication chain is a JSON array, the examples below are presented in that form. See [Transmitting a Chain](authchain.md#transmitting) for more details.
{% endhint %}
...
1. The `type` is [`SIGNER`](authchain.md#SIGNER)
...
1. The type is [`ECDSA_EPHEMERAL`](authchain.md#ECDSA_EPHEMERAL)
...
### Transmitting a Chain <a href="#transmitting" id="transmitting"></a>
...
An example of alternative serialization from within the protocol itself can be found in the [`SignedFetch`](../../runtime/modules/signed_fetch.md) module, which uses a sequence of HTTP headers instead of a JSON array.
...
3. Add the action authorization step:
   1. Set the `type` to a valid value ([see below](authchain.md#types))
...
3. Verify action authorization:
   1. Verify the `type` is a valid value ([see below](authchain.md#types)).
...
### Standard Action Types and Purposes <a href="#types" id="types"></a>
