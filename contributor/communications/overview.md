...
This means that, in addition to the [Archipelago](archipelago) protocol, clients must implement a number of _transports_, each wrapping one of the supported backends in a unified interface.

## Client Life-cycle {#lifecycle}

The life-cycle of a comms client can be summarized in a few steps:

1. **Select a realm**: obtain a URI for the [Archipelago](archipelago) service.
...
Go to the [Transports](transports) or [Archipelago](archipelago) sections to learn more, or a specific transport page for details about it.


## Authentication

Connections to comms are authenticated by having clients sign a server-provided challenge, using the scheme described in the [authentication chain](../auth/authchain) section.

Head over to the [Archipelago](archipelago) page or see a specific transport to learn more about authentication flows.


## Islands

Islands are highly dynamic groups of players that can broadcast messages among themselves, created and maintained by [Archipelago](archipelago) in response to their movements in the world.
...
The flow for getting assigned to an island and joining it is detailed in the [Archipelago](archipelago) section.


## Messages

Messages in the comms protocol are binary blobs serialized using [protocol buffers](https://github.com/protocolbuffers/protobuf), wrapped in a [Packet](messages#Packet) structure.
...
There are several different message types, for a variety of real-time interaction flows. Head over to the [messages](messages) section to learn more.
