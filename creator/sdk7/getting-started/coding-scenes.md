...
See [Entities and components](../architecture/entities-components.md) for an in-depth look of both these concepts and how they're used by Decentraland scenes.
...
See [Custom Components](../architecture/custom-components.md) for more information.
...
See [Get entity by name](../architecture/entities-components.md#get-an-entity-by-name) for more information.

If the entity is a [Smart item](../../scene-editor/interactivity/smart-items.md), you can also call its **Actions** or subscribe to its **Triggers** via code. See [Reference Items](../../scene-editor/code/reference-items.md).
...
See [Systems](../architecture/systems.md) for more details about how systems are used in a scene.
...
In Decentraland scenes, there is no explicitly declared game loop, but rather the [Systems](../architecture/systems.md) of the scene make up the game loop.
...
You can [query components](../architecture/querying-components.md) with the method `engine.getEntitiesWith(...components)` to keep track of all entities in the scene that have certain components.

It often makes sense to query components within a [system](../architecture/systems.md), to then loop over each of the returned entities and perform a same set of actions on each.
...
[Custom Component](../architecture/custom-components.md) definitions are an exception, these must always be written outside the `main()` function, in a separate file. They need to be interpreted before `main()` is executed.
{% endhint %}
...
See [manage dependencies](../libraries/manage-dependencies.md) for more details.
