...
A number of [Event listeners](../interactivity/event-listeners.md) come predefined as part of the SDK, but you can also use the `onChange()` method on any component to achieve the same. This also works with any [Custom Component](custom-components.md) that you defined, without needing any extra work.
...
The same method works out-of-the-box with [Custom Component](custom-components.md). For example:
...
You can also combine this approach with [Querying components](querying-components.md), to bulk-subscribe every entity in the scene that has a certain component to their own function.
