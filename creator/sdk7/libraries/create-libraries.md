...
6. Get an NPM token:
   ...
   2.  Go to **Account > Access Tokens > Generate new Token** Create a new token, give it **Publish** permissions.
...
7.  In your github repository, go to **Settings > Secrets > Actions**, then click **New Repository Secret**.
    ...
    Name your secret **NPM\_TOKEN**, and paste the string from the NPM token as its value.
...
Test your library by using it in a new Decentraland scene while you develop it. Install your npm package into a scene by running `npm i <package-name>`. Then build out a scene that serves for trying out the library's functionality. Try to cover different test cases with your scene, to ensure your library works as expected in every case.
...
1. Add this script to the `scripts` list in package.json: `"link-sdk": "cd node_modules/@dcl/sdk && npm link && cd ../js-runtime && npm link"`
2. Run `npm install`
...
1. Run `npm install`
2. Run `npm run build`
3. Run: `npm link @dcl/sdk @dcl/js-runtime`
