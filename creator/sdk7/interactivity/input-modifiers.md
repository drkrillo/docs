...
You can freeze the player so that none of the input keys can move the avatar. This can be useful for many game mechanics. It's also a good practice to freeze a player while performing an important animation that shouldn't be interrupted by movement, or while a [Virtual Camera](../sdk7/3d-essentials/camera.md) points away from the avatar and you don't want the player to move blindly.
...
* Player inputs don't affect the avatar, but the [global input events](../sdk7/interactivity/button-events/system-based-events.md#global-input-events) can still be listened by the scene. You could use these to control a vehicle, or use a [Virtual Camera](../sdk7/3d-essentials/camera.md) to follow another entity as it moves, treating it as an alternative avatar.
