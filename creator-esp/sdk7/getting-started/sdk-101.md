...
metaLinks:
  alternates:
    - >-
      https://app.gitbook.com/s/oPnXBby9S6MrsW83Y9qZ/sdk7/getting-started/sdk-101
---
...
{% hint style="info" %}
**💡 Tip**: If you prefer to create your scenes with only no-code tools, see [Scene Editor](../../scene-editor/get-started/about-editor.md).
{% endhint %}
...
Read [Installation guide](../../scene-editor/get-started/editor-installation.md) for more details about installing the Creator Hub.
...
Already placed item can be clicked and dragged to reposition them freely. See [Scene editor essentials](../../scene-editor/get-started/scene-editor-essentials.md#position-items) for more details.
...
In the snippet above, we use `engine.getEntityOrNullByName()` to fetch a reference to an entity named _Yellow Crate_. In the next line, we do `if (crate)` to ensure that there really is an entity by this name in the scene. If there isn't one, then the value of `crate` will be `null`. See [Reference Items](../../scene-editor/code/reference-items.md) for more info.
...
{% hint style="info" %}
**💡 Tip**: You can also fetch [Smart Items](../../scene-editor/interactivity/smart-items.md) from your scene in the same way, and do whatever you want with them via code. See [Reference Items](../../scene-editor/code/reference-items.md) for more info.
{% endhint %}
